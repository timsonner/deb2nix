...
class EmitTests(unittest.TestCase):
    ...
    def test_electron_userland_not_sandbox(self) -> None:
        with TemporaryDirectory() as td:
            ...
            self.assertIn('for root in "$out/opt" "$out/share"', package)
            self.assertIn('[ -d "$root" ] || continue', package)
            self.assertIn("ln -sfn", package)
            self.assertIn("ELECTRON_FORCE_IS_PACKAGED", package)
            self.assertIn("ozone-platform-hint=auto", package)
            self.assertIn("makeLibraryPath", package)
            self.assertIn("Prefer the Electron ELF", package)
            self.assertIn("x7fELF", package)
