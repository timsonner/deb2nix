...
def emit_all(out_dir: Path, ctx: EmitContext) -> list[Path]:
    ...
    if ctx.classification.profile != "cli":
...
def _call_args(ctx: EmitContext) -> tuple[list[str], list[str]]:
    """Return (function arguments, buildInputs exprs)."""
    args = ["lib", "stdenv", "dpkg", "autoPatchelfHook", "makeWrapper"]
    if ctx.src_kind == "url":
        args.append("fetchurl")
    inputs = ["stdenv.cc.cc.lib"]
    extra_args: list[str] = []
    for pkg in ctx.mapping.build_inputs:
        top = pkg.split(".", 1)[0]
...
def _emit_cli_package(ctx: EmitContext) -> str:
    ...
    if unmapped_ignore:
        ...
        # The above is list of strings; nix list needs no semicolons
    unfree_comment = ""
    if ctx.unfree:
        unfree_comment = (
            "\n  # UNFREE: Debian license/section is "
            f"{nix_string(ctx.control.license or ctx.control.section or 'unknown')}.\n"
            "  # The generated flake gates this pname with allowUnfreePredicate.\n"
        )
